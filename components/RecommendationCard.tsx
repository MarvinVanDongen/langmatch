...
export default function RecommendationCard({
  ...
  return (
    <article
      className={`
        bg-card rounded-2xl shadow-card transition-shadow hover:shadow-card-hover
        ${isTop ? 'ring-1 ring-accent/20 border border-accent/15' : 'border border-border'}
      `}
    >
      {/* Top strip: rank indicator for #1 */}
      {isTop && (
      ...
      <div className="p-5 pt-4 space-y-4">
        ...
        <div className="space-y-2.5">
          ...
          <ScoreBar label="Waarde" score={school.value_score} color="value" />
          <ScoreBar label="Vertrouwen" score={school.trust_score} color="trust" />
        </div>
        ...
        <div className="flex gap-2.5 pt-1">
          <Link
            href={detailUrl}
            className="flex-1 text-center text-sm font-medium px-4 py-2.5 rounded-xl border border-border text-ink hover:border-accent/50 hover:bg-accent-light transition-colors"
          >
            Bekijk school
          </Link>
          <Link
            href={bookUrl}
            className={`flex-1 text-center text-sm font-medium px-4 py-2.5 rounded-xl transition-colors ${
              isTop
                ? 'bg-accent text-white hover:bg-accent-hover'
                : 'bg-accent/90 text-white hover:bg-accent'
            }`}
          >
            {school.cashback_possible ? `Boek met €${school.cashback_amount}` : 'Boek nu'}
          </Link>
        </div>
      </div>
    </article>
